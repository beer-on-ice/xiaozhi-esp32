        ...
        .callback =
            [](void* arg) {
                ...
                if (app.GetDeviceState() == kDeviceStateIdle) {
                    ...
                    app.Schedule([protocol]() { protocol->StartMqttClient(false); });
                }
            },
        .arg = this,
    };
...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    mqtt_->OnDisconnected([this]() {
        if (scb.faceAction_chin_handle_ != nullptr) {
            ESP_LOGI(TAG, "MQTT 连接断开,停止 DM0 舵机");
            vTaskSuspend(scb.faceAction_chin_handle_); // W_TODO: 在断开连接时停止舵机
        }
...
void MqttProtocol::Myself_SendText(const std::string& text) {
    SendText(text);
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        int ret =
            mbedtls_aes_crypt_ctr(&aes_ctx_, decrypted_size, &nc_off, nonce, stream_block, encrypted, (uint8_t*)packet->payload.data());
        if (ret != 0) {
...
void MqttProtocol::ParseServerHello(const cJSON* root) { auto transport = cJSON_GetObjectItem(root, "transport");
...
bool MqttProtocol::IsAudioChannelOpened() const { return udp_ != nullptr && !error_occurred_ && !IsTimeout(); }
