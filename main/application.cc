...
static const char* const STATE_STRINGS[] = {"unknown",  "starting",  "configuring", "idle",          "connecting",  "listening",
                                            "speaking", "upgrading", "activating",  "audio_testing", "fatal_error", "invalid_state"};
...
void Application::ShowActivationCode(const std::string& code, const std::string& message) {
    ...
    static const std::array<digit_sound, 10> digit_sounds{{digit_sound{'0', Lang::Sounds::OGG_0}, digit_sound{'1', Lang::Sounds::OGG_1},
...
void Application::Start() {
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(text)) {
                ESP_LOGI(TAG, ">> %s", text->valuestring);
                Schedule([this, display, message = std::string(text->valuestring)]() { display->SetChatMessage("user", message.c_str()); });
            }
    ...
    scb.init_header(); // W_TODO
    // scb.init_esp_now();
    scb.init_mqtt();
}
// Add a async task to MainLoop 在主循环中添加一个异步任务
 ...
 * If other tasks need to access the websocket or chat state, they should use Schedule to call this function
 * 主事件循环负责控制聊天状态以及 WebSocket 连接
 * 如果其他任务需要访问 WebSocket 或聊天状态, 它们应当使用 Schedule 来调用此函数
 */
...
void Application::SetDeviceState(DeviceState state) {
    ...
    switch (state) {
        ...
        //     ESP_LOGW(TAG, "11.===");
        //     scb.udp_send_logger("发(∪.∪ )...zzz待命...");
        //     ESP_LOGI(TAG, "SetDeviceState -> kDeviceStateUnknown || kDeviceStateIdle");
        ...
        case kDeviceStateConnecting:
            scb.mqtt_publish("连接中...");
            ESP_LOGI(TAG, "SetDeviceState -> kDeviceStateConnecting");
        ...
        case kDeviceStateListening:
            scb.mqtt_publish("聆听中...");
            ESP_LOGI(TAG, "SetDeviceState -> kDeviceStateListening");
            if (scb.faceAction_chin_handle_ != nullptr) {
                vTaskSuspend(scb.faceAction_chin_handle_); // W_TODO
            }
        ...
        case kDeviceStateSpeaking:
            scb.mqtt_publish("说话中...");
            ESP_LOGI(TAG, "SetDeviceState -> kDeviceStateSpeaking");
            ...
            if (scb.faceAction_chin_handle_ != nullptr) {
                vTaskResume(scb.faceAction_chin_handle_); // W_TODO
            }
            break;
        default:
            ESP_LOGI(TAG, "kDeviceStateUnknown");
            break;
    }
}
// 重启
...
void Application::SendMcpMessage(const std::string& payload) {
    ...
    if (xTaskGetCurrentTaskHandle() == main_event_loop_task_handle_) {
        ...
        Schedule([this, payload = std::move(payload)]() { protocol_->SendMcpMessage(payload); });
    }
}
// 设置音频编码
